...
- Discord — провайдер добавлен в v1.2 (BETA, self-bot по QR — ADR-019/020).
  ...
  вкладки Медиа/Файлы поверх того же поиска с фильтром `has` (ADR-023), и
  @-mention автокомплит через gateway `members.fetch` (ADR-024, вставляет ник
  текстом — функциональный пинг `<@id>` отложен).
  Плюс функциональные @-пинги — выбранные из попапа упоминания уходят как
  `<@id>` (ADR-025; пилл-UI в композере отложен, но пинг работает).
  **Остаётся:** глобальный поиск (у Discord нет cross-guild endpoint —
  итерировать серверы), mention-пиллы в композере (косметика), историческое
  непрочитанное (read-state / `acknowledge` в либе нет), poll, тумблер mute в
  карточке (появится, когда будет метод), mute для DM. Плюс: отправка —
  best-effort из-за CAPTCHA (ADR-019).
  **Реакции** — вынесены в отдельную провайдеро-агностичную задачу (все три
  либы умеют), планово **перед следующим провайдером**; детали в
