# Не делать до версии 1.0

- AI
- GitHub
- Jira
- Copilot
- MCP
- Голосовые сообщения (запись/отправка; инлайн-прослушивание — см. ADR-005,
  пока доступны как скачиваемый файл)
- Темы оформления
- Видеозвонки

## Отложено после 1.0

- Единое дерево нескольких провайдеров одновременно + `providerId` в моделях
  (ADR-009).
- WhatsApp: shared-media, поиск, профили (ADR-010). Тумблер mute — с
  карточкой профиля (mute уже уважается read-only, ADR-013).
  Сделаны в v1.1: пагинация истории (ADR-011), read-receipts (ADR-012),
  read-only mute (ADR-013), editor-парити `sendCode`/`sendFile` (ADR-014),
  входящие медиа (ADR-016), аватары (ADR-017).
- Discord — провайдер добавлен в v1.2 (BETA, self-bot по QR — ADR-019/020).
  Сделано: чаты/серверы/треды, история, realtime, богатый рендер (markdown/
  embeds/Components V2/форварды/системные), медиа, editor-шаринг; + полиш:
  аватары авторов, разделение Direct Messages/серверы, иконки по типу (`#`
  каналы), **профиль-карточки** (личка/группа/канал/автор).
  Плюс read-only mute — уважаем mute серверов/каналов из офиц. приложения
  (тихо + 🔇), без тумблера; DM либа не отдаёт (ADR-021). Плюс поиск в чате —
  `searchMessages` через `channel.messages.search` (ADR-022), и shared media —
  вкладки Медиа/Файлы поверх того же поиска с фильтром `has` (ADR-023), и
  @-mention автокомплит через gateway `members.fetch` (ADR-024, вставляет ник
  текстом — функциональный пинг `<@id>` отложен).
  **Остаётся:** глобальный поиск (у Discord нет cross-guild endpoint —
  итерировать серверы), функциональные @-пинги (`<@id>` + пиллы в композере),
  историческое непрочитанное (read-state / `acknowledge` в либе нет),
  poll/reactions, тумблер mute в карточке (появится, когда будет метод), mute
  для DM. Плюс: отправка — best-effort из-за CAPTCHA (ADR-019).
